...
- **Cloudflare:** Use Case: Edge rate limiting for 25M+ internet properties; Scale: Billions of req/sec
- **Stripe:** Use Case: API rate limiting per merchant; Scale: Millions of API calls/day
- **Google:** Use Case: Cloud API quotas per project; Scale: Millions of projects
- **Netflix:** Use Case: Zuul gateway rate limiting; Scale: 100B+ requests/day
- **Amazon:** Use Case: API Gateway throttling per API key; Scale: Millions of APIs
...
- **Latency overhead:** < 1ms p99; Rationale: Rate limiter sits in the hot path of every request
- **Throughput:** 10M+ decisions/sec; Rationale: Aggregated across all servers in the fleet
- **Availability:** 99.99% (52 min/year); Rationale: Downtime means either all traffic blocked or all allowed
- **Consistency:** Near-exact (< 1% drift); Rationale: Slight over-counting acceptable; under-counting is not
- **Scalability:** Horizontal; Rationale: Add more Redis nodes / servers linearly
- **Fault tolerance:** Configurable fail mode; Rationale: Fail-open (allow) or fail-closed (deny) per rule
- **Durability:** Rules durable, counters ephemeral; Rationale: Counters naturally expire; rules must survive restarts
...
- **Rate Limit Middleware:** Intercepts every request, calls rate limit logic; Tech Choice: In-process library
- **Rule Cache:** Local in-memory cache of rate limit rules; Tech Choice: Caffeine / Guava
- **Rate Limit Service:** Core logic: algorithm execution, counter management; Tech Choice: Java / Go library
- **Redis Cluster:** Distributed counter storage with atomic operations; Tech Choice: Redis 7+ Cluster
- **Rules Service:** CRUD API for rate limit rules, pushes updates; Tech Choice: REST microservice
- **Rules Database:** Persistent storage for rate limit configurations; Tech Choice: PostgreSQL
- **Admin Dashboard:** UI for managing rules, viewing usage, analytics; Tech Choice: React + REST
- **Analytics Pipeline:** Collects rate limit events for monitoring and reporting; Tech Choice: Kafka + ClickHouse
