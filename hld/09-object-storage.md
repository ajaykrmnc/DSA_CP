...
- **API Server:** Parse HTTP, auth routing, erasure encode/decode, stream data; Stateless?: Yes; Scale Strategy: Horizontal auto-scale
- **IAM / Auth Service:** Evaluate bucket policies, ACLs, IAM policies, signature verification; Stateless?: Yes; Scale Strategy: Cache policies locally
- **Metadata Service:** CRUD for object/bucket metadata, listing queries; Stateless?: Stateless tier; stateful store; Scale Strategy: Shard by bucket hash
- **Namespace Service:** Global bucket name uniqueness, bucket-to-region mapping; Stateless?: Stateless tier; backed by global DB; Scale Strategy: Small, replicated
- **Placement Service:** Map objects to data node sets using CRUSH-like algorithm; Stateless?: Yes (reads placement map); Scale Strategy: Replicate placement map
- **Data Service:** Route data reads/writes to correct data nodes; Stateless?: Yes; Scale Strategy: Horizontal
- **Data Node:** Store/retrieve erasure-coded chunks on raw disks; Stateless?: Stateful; Scale Strategy: Add nodes, rebalance
- **Garbage Collector:** Reclaim space from deleted objects, incomplete multipart uploads; Stateless?: Background daemon; Scale Strategy: Partition work by node
- **Data Compactor:** Compact fragmented extent files, reclaim gaps; Stateless?: Background daemon; Scale Strategy: Per-node
- **Integrity Scrubber:** Periodically verify chunk checksums, trigger repair; Stateless?: Background daemon; Scale Strategy: Per-node
