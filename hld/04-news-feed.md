...
- **Latency:** Feed generation <200ms; Rationale: Users expect instant feed loading
- **Availability:** 99.99% uptime; Rationale: Core engagement feature; downtime = revenue loss
- **Consistency:** Eventual (seconds); Rationale: Feed does not need strong consistency
- **Throughput:** ~6K feed reads/sec; Rationale: Based on 500M DAU estimation
- **Durability:** No data loss for posts; Rationale: Posts are user-generated content; must persist
- **Scalability:** Handle 500M DAU; Rationale: Must scale horizontally
- **Global Reach:** Multi-region; Rationale: Users are worldwide; CDN for media
- **Fan-out Handling:** Celebrity w/ 100M followers; Rationale: Must not block on massive fan-out
...
- **Total registered users:** Value: 2 billion; Derivation: Given
- **Daily Active Users (DAU):** Value: 500 million; Derivation: ~25% of total
- **Avg posts per user per day:** Value: 2; Derivation: Most users post less, power users more
- **Total posts per day:** Value: 1 billion; Derivation: 500M x 2
- **Avg followers per user:** Value: 300; Derivation: Power-law distribution
- **Feed refreshes per user/day:** Value: 10; Derivation: Open app ~10 times
- **Avg feed page size:** Value: 20 posts; Derivation: Per page/scroll
...
- **Ingress:** Calculation: 35K posts/sec × 1.5KB + media uploads; Result: ~500 MB/s
- **Egress:** Calculation: 174K feed reads/sec × 20 posts × 1.5KB; Result: ~5 GB/s (text)
- **Media CDN:** Calculation: Images + video serving; Result: ~50-100 GB/s
...
- **`POST /feed/posts`:** Rate Limit: 50 requests; Window: 1 hour
- **`GET /feed`:** Rate Limit: 300 requests; Window: 1 min
- **`POST /like`:** Rate Limit: 200 requests; Window: 1 hour
- **`POST /follow`:** Rate Limit: 100 requests; Window: 1 hour
- **`POST /comments`:** Rate Limit: 100 requests; Window: 1 hour
...
- **API Gateway:** Responsibilities: Auth, rate limiting, request routing, SSL termination; Scale: 50+ instances
- **Post Service:** Responsibilities: CRUD for posts, validation, media orchestration; Scale: 100+ instances
- **Feed Service:** Responsibilities: Feed retrieval, celebrity merge, pagination; Scale: 200+ instances
- **Fan-out Service:** Responsibilities: Distribute new posts to follower feed caches; Scale: 500+ workers
- **Social Graph Svc:** Responsibilities: Follow/unfollow, follower lists, recommendations; Scale: 50+ instances
- **Media Service:** Responsibilities: Upload, transcode, thumbnail generation; Scale: 100+ instances
- **Notification Svc:** Responsibilities: Push notifications for interactions; Scale: 50+ instances
- **Ranking Service:** Responsibilities: ML-based feed ranking and personalization; Scale: 100+ instances
- **Trending Service:** Responsibilities: Compute trending hashtags from event stream; Scale: 20+ instances
...
- **Ordering:** Chronological: Newest first; Ranked (ML-based): Relevance score
- **User expectation:** Chronological: Simple, predictable; Ranked (ML-based): "Best" content surfaces
- **Engagement:** Chronological: Lower; Ranked (ML-based): Higher (typically +10-30%)
- **Cold start:** Chronological: Works perfectly; Ranked (ML-based): Needs interaction history
- **Transparency:** Chronological: High (user understands); Ranked (ML-based): Low (black box)
- **Implementation:** Chronological: Simple; Ranked (ML-based): Complex (ML pipeline)
- **Celebrity advantage:** Chronological: Low (buried quickly); Ranked (ML-based): High (engagement begets visibility)
- **Used by:** Chronological: Mastodon, RSS readers; Ranked (ML-based): Facebook, Twitter, Instagram, TikTok
...
- **Stability:** Cursor-Based (selected): Stable on inserts; Offset-Based: Rows shift on inserts
- **Performance:** Cursor-Based (selected): O(1) with index; Offset-Based: O(offset) scan
- **Infinite scroll:** Cursor-Based (selected): Perfect fit; Offset-Based: Works but fragile
- **Jump to page N:** Cursor-Based (selected): Not supported; Offset-Based: Supported
- **Cache-friendliness:** Cursor-Based (selected): High (immutable cursor); Offset-Based: Low (offset changes)
