...
- **Load Balancer / CDN:** Route queries to nearest DC, cache static assets; Scale: Global, 20+ DCs
- **Query Parser:** Tokenize, normalize, spell-check, classify intent; Scale: Stateless, ~5K instances
- **Query Cache:** Cache frequent query results (LRU); Scale: ~100 TB distributed cache
- **Index Servers:** Store index shards, execute BM25 retrieval; Scale: ~30K machines
- **Ranker / Re-ranker:** ML-based relevance scoring; Scale: Co-located with index or separate tier
- **Snippet Generator:** Extract relevant text from stored documents; Scale: Stateless workers
- **Result Compiler:** Assemble final response (results + panels + suggestions); Scale: Stateless workers
- **Autocomplete Service:** Trie-based prefix matching on query logs; Scale: In-memory, replicated
- **Spell Correction:** Edit-distance computation, language models; Scale: In-memory, replicated
