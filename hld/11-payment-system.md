...
- **Payment orders:** Database: PostgreSQL (primary); Rationale: ACID transactions, strong consistency, mature ecosystem
- **Ledger entries:** Database: PostgreSQL (separate cluster); Rationale: Double-entry bookkeeping requires ACID, append-only
- **Payment methods (vault):** Database: PostgreSQL + HSM; Rationale: Encrypted at column level, tokenized references
- **Merchant config:** Database: PostgreSQL + Redis cache; Rationale: Relatively static, frequently read
- **Idempotency keys:** Database: Redis (primary) + PostgreSQL (backup); Rationale: Fast lookup, 24h TTL, persistence for crash recovery
- **Webhook events:** Database: PostgreSQL + Kafka; Rationale: Ordered, durable event stream for reliable delivery
- **Analytics/reporting:** Database: ClickHouse or BigQuery; Rationale: Columnar, optimized for aggregation queries
- **Audit logs:** Database: Append-only PostgreSQL / S3; Rationale: Immutable, tamper-evident, long retention
