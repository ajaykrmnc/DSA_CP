...
- **String:** Operations: GET, SET, INCR, APPEND; Use Case: Counters, simple caching
- **Hash:** Operations: HGET, HSET, HMGET, HGETALL; Use Case: Object storage, user profiles
- **List:** Operations: LPUSH, RPUSH, LPOP, RPOP, LRANGE; Use Case: Message queues, activity feeds
- **Set:** Operations: SADD, SREM, SMEMBERS, SINTER; Use Case: Tags, unique visitors
- **Sorted Set:** Operations: ZADD, ZRANGE, ZRANGEBYSCORE; Use Case: Leaderboards, rate limiters
- **HyperLogLog:** Operations: PFADD, PFCOUNT, PFMERGE; Use Case: Cardinality estimation
- **Bitmap:** Operations: SETBIT, GETBIT, BITCOUNT; Use Case: Feature flags, daily active users
- **Stream:** Operations: XADD, XREAD, XRANGE; Use Case: Event sourcing, message streams
...
- **RDB:** Mechanism: Point-in-time snapshots to disk; Pros: Compact, fast recovery; Cons: Data loss between snapshots
- **AOF:** Mechanism: Append every write command to log; Pros: Minimal data loss; Cons: Larger files, slower restart
- **Hybrid:** Mechanism: RDB snapshot + AOF tail; Pros: Best of both worlds; Cons: More complex management
- **None:** Mechanism: Pure in-memory, no persistence; Pros: Maximum performance; Cons: All data lost on restart
...
- **Avalanche:** Large number of keys expire simultaneously, overwhelming the database; Cause: TTLs set to same value (e.g., midnight expiry); Solution: Add random jitter to TTL: `TTL = base + random(0, spread)`
- **Stampede:** Single hot key expires, many concurrent requests hit DB; Cause: Popular key expiration; Solution: Mutex lock (SETNX), probabilistic early refresh, background pre-warming
- **Penetration:** Requests for keys that will NEVER exist in cache or DB; Cause: Attacker queries non-existent IDs; Solution: Bloom filter to check existence, cache null results with short TTL, input validation
...
- **Meaning:** MOVED: "Slot X permanently lives on Node Y"; ASK: "Slot X is being migrated; try Node Y for this key"
- **Client action:** MOVED: Update slot map permanently, retry to Node Y; ASK: Send ASKING + command to Node Y, do NOT update slot map
- **When seen:** MOVED: After resharding is complete, or client has stale map; ASK: During active slot migration
- **Frequency:** MOVED: Should be rare after initial discovery; ASK: Temporary, only during migration
