...
- **Latency:** p50 < 50ms, p99 < 100ms; Rationale: Each keystroke triggers a call
- **Availability:** 99.99% (52 min downtime/year); Rationale: Critical path of every search
- **Throughput:** 500K+ QPS sustained; Rationale: 40B requests/day
- **Consistency:** Eventual (seconds to minutes); Rationale: Stale suggestions acceptable
- **Scalability:** Horizontal to 1M+ QPS; Rationale: Handle spikes (Super Bowl, elections)
- **Data freshness:** Trending < 5 min, Batch < 1 hour; Rationale: Trending topics must surface quickly
...
- **`prefix`:** Type: string; Required: Yes; Default: -; Description: Typed prefix (URL-encoded, UTF-8)
- **`user_id`:** Type: string; Required: No; Default: -; Description: For personalized suggestions
- **`limit`:** Type: int; Required: No; Default: 10; Description: Number of suggestions (max 20)
- **`lang`:** Type: string; Required: No; Default: "en"; Description: Language code (ISO 639-1)
- **`category`:** Type: string; Required: No; Default: "all"; Description: Scope: all, news, images, shopping
- **`lat`, `lon`:** Type: float; Required: No; Default: -; Description: For geo-aware suggestions
...
- **High latency:** Condition: p99 > 200ms for 2 min; Severity: P1
- **Low CDN hit rate:** Condition: < 60% for 10 min; Severity: P2
- **Trie version skew:** Condition: > 2 versions for 30 min; Severity: P2
- **Trie rebuild failure:** Condition: 2 consecutive failures; Severity: P1
- **Shard replicas < 2:** Condition: Any shard; Severity: P1
- **Error rate:** Condition: > 0.1% 5xx for 2 min; Severity: P1
- **CTR drop:** Condition: > 20% vs 7-day avg; Severity: P2
- **Offensive content spike:** Condition: > 5x normal filtered; Severity: P1
...
- **Prefix lookup:** Trie: O(L) -- excellent; Inverted Index: O(log N) with sorted lists
- **Pre-computed top-K:** Trie: Natural fit at each node; Inverted Index: Requires separate index
- **Fuzzy matching:** Trie: Needs edit-distance walk; Inverted Index: Better (n-gram based)
- **Multi-word:** Trie: Full query as key; Inverted Index: Per-word + intersection
- **Best for:** Trie: Exact prefix, real-time; Inverted Index: Fuzzy, multi-field search
...
- **Freshness:** Real-Time: Seconds; Batch: Minutes to hours
- **Complexity:** Real-Time: High (concurrent R/W); Batch: Low (rebuild and swap)
- **Risk:** Real-Time: Memory fragmentation; Batch: Stale data between builds
...
- **Aggressive (long TTL):** Pro: 95%+ hit rate, low load; Con: Stale suggestions
- **Conservative (short):** Pro: Fresh results; Con: Higher trie load
- **Per-user caching:** Pro: Personalized + cached; Con: Huge cache, low per-user hits
- **Two-tier (recommended):** Pro: CDN for global, compute personal overlay; Con: Slightly more logic
