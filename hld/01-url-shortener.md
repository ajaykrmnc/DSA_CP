...
- **Latency:** p50 < 5ms, p99 < 50ms redirect; Rationale: Redirect must feel instant; any delay loses users.
- **Throughput:** 50K reads/sec, 500 writes/sec; Rationale: Read-heavy workload (100:1 read-to-write ratio).
- **Availability:** 99.99% (52 min downtime/year); Rationale: Revenue-generating links must always resolve; use multi-region.
- **Consistency:** Eventual consistency (reads); Rationale: A newly created short URL may take ~1-2s to propagate to all replicas.
- **Durability:** 99.999999999% (11 nines); Rationale: Once a URL is stored, it must not be lost. Replicate across AZs/regions.
- **Scalability:** 100B+ URLs over 5 years; Rationale: Must handle internet-scale growth without re-architecture.
- **Security:** No enumeration attacks; Rationale: Short keys must not be sequential/predictable.
