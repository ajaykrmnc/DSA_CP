...
query, find the GCD (Greatest Common Divisor) of all elements in a given range [L, R].

The segment tree approach divides the array into segments and stores the GCD of each segment. Since GCD is associative
(gcd(a, gcd(b, c)) = gcd(gcd(a, b), c)),
we can combine GCD values from child segments. Both range queries and point updates can be performed in O(log n)
time complexity, making this approach efficient for multiple operations.
