...
at index i. For each element, check all previous elements and extend the longest subsequence where current element is
greater.

Time complexity is O(n²). An optimized O(n log n) solution uses binary search with a temporary array to maintain the
smallest tail.
