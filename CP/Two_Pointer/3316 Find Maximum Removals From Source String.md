...
Use DP over source and pattern positions. At each source index, either remove it if allowed or keep it to match/skip,
preserving subsequence feasibility.
...
public:
  int recur(string &source, string &pattern, set<int>&st, int i, int j, vector<vector<int>>&dp) {
    ...
    if(source[i] == pattern[j]) {
      ...
      else return dp[i][j] = max(1 + recur(source, pattern,st, i + 1, j, dp), recur(source, pattern,st, i + 1, j + 1,
      																																							dp));
    }
    if(st.find(i) != st.end())
