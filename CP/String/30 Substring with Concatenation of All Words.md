...
Find every starting index where the substring is exactly a concatenation of all given words once.

## Approach

All words have the same length, so scan by word-length offsets. Maintain a frequency map for the current block window and shrink whenever a word is overused or invalid.
