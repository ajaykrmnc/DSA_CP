# is Graph Bipartitite

There is an **undirected** graph with `n` nodes, where each node is numbered between `0` and `n - 1`. You are given a 2D
array `graph`, where `graph[u]` is an array of nodes that node `u` is adjacent to. More formally, for
each `v` in `graph[u]`, there is an undirected edge between node `u` and node `v`. The graph has the following
properties:
...
public:
  bool isBipartite(vector<vector<int>>& graph) {
    ...
    function<bool(int,int)>dfs = [&](int src,int clr){
      bool flag = 1;
      color[src] = clr;
      for(auto node: graph[src]){
        if(color[node] == -1){
          flag = flag & dfs(node, 1- clr);
        }
        else{
          if(color[node] == clr){
            flag = 0;
