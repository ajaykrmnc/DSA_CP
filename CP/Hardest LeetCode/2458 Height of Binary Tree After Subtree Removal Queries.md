...
**Difficulty:** Hard  
**Pattern:** Tree rerooting / level maxima  
...
Precompute subtree heights and depth. For each depth level, remember the two largest subtree heights; removing a node uses the best height at its depth that does not belong to that node.
